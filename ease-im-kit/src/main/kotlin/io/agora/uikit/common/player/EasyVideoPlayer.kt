 ...
 */
class EasyVideoPlayer @JvmOverloads constructor(
    private val context: Context,
    private val attrs: AttributeSet? = null,
    private val defStyleAttr: Int = 0
) : FrameLayout(context, attrs, defStyleAttr), EasyIUserMethods, TextureView.SurfaceTextureListener,
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        ...
        if (attrs != null) {
            val a: TypedArray =
                context.theme.obtainStyledAttributes(attrs, R.styleable.EasyVideoPlayer, 0, 0)
            try {
                val source: String? = a.getString(R.styleable.EasyVideoPlayer_easy_source)
                if (source != null && !source.trim { it <= ' ' }.isEmpty()) mSource =
                    Uri.parse(source)
                val playDrawableResId: Int =
                    a.getResourceId(R.styleable.EasyVideoPlayer_easy_playDrawable, -1)
                val pauseDrawableResId: Int =
                    a.getResourceId(R.styleable.EasyVideoPlayer_easy_pauseDrawable, -1)
                ...
                mControlsDisabled =
                    a.getBoolean(R.styleable.EasyVideoPlayer_easy_disableControls, false)
                mThemeColor = a.getColor(
                    R.styleable.EasyVideoPlayer_easy_themeColor,
                    Util.resolveColor(context, android.R.attr.colorPrimary)
                )
                mAutoFullscreen =
                    a.getBoolean(R.styleable.EasyVideoPlayer_easy_autoFullscreen, false)
    ...
    protected override fun onFinishInflate() {
        ...
        if (mControlsDisabled) {
            ...
            mClickFrame?.setOnClickListener {
                toggleControls()
                if (mCallback != null) {
                    mCallback?.onClickVideoFrame(this@EasyVideoPlayer)
                }
            }
        }
        invalidateThemeColors()
