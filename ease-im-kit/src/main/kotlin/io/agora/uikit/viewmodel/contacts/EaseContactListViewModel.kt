        ...
        viewModelScope.launch {
            flow {
                ...
                .collect { result ->
                    ...
                    val conversation:ChatConversation? = ChatClient.getInstance().chatManager().getConversation(conversationId,conversationType)
                    result?.let {
                        conversation?.parse()?.setSilent(it.isConversationRemindTypeEnabled)
        ...
        viewModelScope.launch {
            flow {
                ...
                .collectWithCheckErrorCode {
                    ...
                    val conversation:ChatConversation? = ChatClient.getInstance().chatManager().getConversation(conversationId,conversationType)
                    conversation?.parse()?.setSilent(false)
